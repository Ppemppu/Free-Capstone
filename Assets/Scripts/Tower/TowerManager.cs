...
public class TowerManager : MonoBehaviour
{
    ...
    private Dictionary<TowerRank, float> weight = new Dictionary<TowerRank, float>
    {  //타워 등급 확률
    ...
    public TowerData GetRandomTowerPrefab()
    {   //타워 랜덤 설정 가중치 랜덤 알고리즘
        float randomValue = UnityEngine.Random.value;
        float total = 0.0f; 
        foreach (TowerRank rank in weight.Keys)
        {
            total += weight[rank]; //가중치 누적
